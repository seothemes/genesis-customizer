...
class Admin_API {
	...
	public static function display_field( $data = [] ) {
		...
		switch ( $field['type'] ) {
			...
			case 'color':
				?>
                <input type="text" name="<?php esc_attr_e( $option_name ); ?>" class="color-picker" title="<?php esc_attr_e( $field['label'] ); ?>" value="<?php esc_attr_e( $data ); ?>" data-show-opacity="true" data-default-color="<?php esc_attr_e( $field['default'] ) ?>"/>
				<?php
				break;
